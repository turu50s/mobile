...
if (!preg_match("/^[1-9][0-9]*$/",$_POST['number1'])) {
    ...
    HTTP::redirect("http://www.asa-kashiwa.net/mobile/seibu/sb.php?".$sis_id);   
...
<img src="./mini.gif" width="178" height="31" alt="" style="margin:7px 0 7px 0;" />
</div>
<!--ロゴEND-->
...
・以下の項目に入力後、送信して下さい。<br /><br /> 
      ...
      <font color="red">※</font>&#xE21Cお名前：<br>
      <input type="text" name="name" value="<?php print "$name"; ?>" mode="hiragana" accesskey="1" size="20"><br>
　&#xE21Dペンネーム：<br>
  <input type="text" name="pname" value="<?php print "$pname"; ?>" mode="hiragana" accesskey="2" size="20"><br>
　&#xE21E年齢：
...
?>
   ...
   <font color="red">※</font>&#xE21Fご住所:柏市<br>
...
try {
    ...
    $db = new PDO('sqlite2:../../SQLiteManager-1.2.0/mobile');
    $db->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
    
    $sql = "SELECT * FROM 'address' WHERE tenpo = 'seibu'";
    $rs  = $db->query($sql);
...
?>
  ...
  　番地
     <input type="text" name="street" value="<?php print "$street"; ?>" mode="numeric" size="20"><br>
  　建物
     <input type="text" name="bilding" value="<?php print "$bilding"; ?>" mode="hiragana" size="20"><br>
   <font color="red">※</font>&#xE220電話：<br>
      <input type="text" name="tel" value="<?php print "$tel"; ?>" mode="numeric" accesskey="5" size="20"><br>
       ※ﾒｰﾙｱﾄﾞﾚｽは、半角で入力して下さい。<br>
   <font color="red">※</font>&#xE221ﾒｰﾙｱﾄﾞﾚｽ：<br>
      <input type="text" name="email1" value="<?php print "$email1"; ?>" mode="alphabet" accesskey="6" size="20">@
   <select name="email2" >
   <option value="">選択して下さい</option>
<?php   
  try {
...
　&#xE222コメント：<br>
          <textarea name="comment" cols="20" rows="2" mode="hiragana" accesskey="7"><?php print "$comment"; ?></textarea><br>
          <input type="submit" value="確認する">
          
      </form>
      <a href="sb.php">&#xE04Aチケット選択画面へ戻る。</a>
<?php
}
...
?>
        ...
        <form action="<?php echo $ssl.$_SERVER["PHP_SELF"]; ?>" method="post">
           &#xE21Cお名前：<?php print "$name"; ?><br>
           &#xE21Dペンネーム：<?php print "$pname"; ?><br>
           &#xE21E年齢：<?php print "$age"; ?>歳<br>
           &#xE21Fご住所：<?php print "$town"; ?><?php print "$street"; ?><br>
            <?php if ($bilding != "") { print "　　　　　"."$bilding<br>"; } ?>
           &#xE220電話：<?php print "$tel"; ?><br>
           &#xE221メール：<br><?php print "　"."$email"; ?><br>
           &#xE222チケット番号：<br>
   　　第1希望  <?php print $number1 ?><br>
   　　第2希望  <?php print $number2 ?><br>
   　　第3希望  <?php print $number3 ?><br>
          &#xE223コメント：<br><?php print "$comment1"; ?>
...
<?php
    ...
    $to = "seibu-tk@asa-kashiwa.net";
        
    mb_language("Japanese");
    mb_internal_encoding ("UTF-8");
         
    $title = mb_convert_encoding("【ASAのミニ情報紙】チケットプレゼント申込",mb_internal_encoding() ,"SJIS-win");
