...
if (!preg_match("/^[1-9][0-9]*$/",$_POST['number1'])) {
    ...
    HTTP::redirect("http://www.asa-kashiwa.net/mobile/seibu/fon.php?".$sis_id);   
...
・以下の項目に入力後、送信して下さい。<br /><br /> 
      ...
      <form action="<?php echo $ssl.$_SERVER["PHP_SELF"]; ?>" method="post" accept-charset="UTF-8">
...
try {
    ...
    while ($row = $rs->fetch(PDO::FETCH_ASSOC)) {
        print('<option value="' . mb_convert_encoding($row['town'],"UTF-8",mb_internal_encoding()) .'">'
            . mb_convert_encoding($row['town'],"UTF-8",mb_internal_encoding()) . '</option>');  
    }
...
<?php
    ...
    $title = mb_convert_encoding("【ASAのミニ情報紙】チケットプレゼント申込",mb_internal_encoding() ,"UTF-8");
    ...
    if ($name != "" && $address != "" && $email != "") {
       if (mb_send_mail($to, $title, mb_convert_encoding($naiyo,mb_internal_encoding() ,"UTF-8"), $header)) {
          print "  ";
       } else {
          print "**** メール送信失敗です。  ****";
       }
    ...
    $title = mb_convert_encoding("チケットプレゼントの申込",mb_internal_encoding() ,"UTF-8");
    $date  = date("Y/m/d H:i:s (D)");
