...
if (!preg_match("/^[1-9][0-9]*$/",$_POST['number1'])) {
    ...
    HTTP::redirect("http://www.asa-kashiwa.net/mobile/chuo/fon.php?".$sis_id);   
...
try {
    ...
    while ($row = $rs->fetch(PDO::FETCH_ASSOC)) {
        print('<option value="' . $row['town'] .'">' . $row['town'] . '</option>');
...
<?php
    ...
    $title = mb_convert_encoding("【ASAフレンズ】チケットプレゼント申込",mb_internal_encoding() ,"UTF-8");
